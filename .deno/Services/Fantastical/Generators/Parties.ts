...
import { mysticOrder, militaryUnit, guilds } from "../Resources/Parties.ts";
...
export class Parties {
  ...
  public guild(): string {
    ...
    if (roll < 6) {
      return `${capitalize(sample(guilds.adjectives))} ${capitalize(sample(guilds.actions))} ${capitalize(sample(guilds.titles))}`;
    }
